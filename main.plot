...
plot sprintf(s, 'no_lpt', 'signal', 'lepton p_t') u ($1<25*3.0/2?$1*2.0/3:1/0):2 notitle, \
     sprintf(s, 'normal', 'signal', 'lepton p_t') notitle

set title '$\Z\to ee$'
set output 'grafiken/zee/lpt_vs_no_lpt.tikz'
set yrange [0:100]
plot sprintf(s, 'no_lpt', 'zee', 'lepton p_t') u ($1<25*3.0/2?$1*2.0/3:1/0):2 notitle, \
     sprintf(s, 'normal', 'zee', 'lepton p_t') notitle

set title '$\Z\to\mu\mu$'
set output 'grafiken/zmumu/lpt_vs_no_lpt.tikz'
plot sprintf(s, 'no_lpt', 'zmumu', 'lepton p_t') u ($1<25.0*3.0/2?$1*2.0/3:1/0):2 notitle, \
...
set mxtics pi/8
f(x) = 4/100.0 * x - 4
set xtics ('$-\pi$' f(-pi), '$-\frac\pi 2$' f(-pi/2), '$0$' f(0), '$\frac\pi 2$'\
        f(pi/2), '$\pi$' f(pi))
...
set terminal lua solid size 11cm, 8cm nobitmap font '\small'
...
set title 'Signal'
splot sprintf(s, 'normal', 'signal', 'El Mu N') u ($1-1):($2-1):3 notitle w image
set output 'grafiken/ttbar/el_mu_n.tikz'
set title '$t\bar{t}$'
splot sprintf(s, 'normal', 'ttbar', 'El Mu N') u ($1-1):($2-1):3 notitle w image
set output 'grafiken/zee/el_mu_n.tikz'
set title '$\Z\to ee$'
splot sprintf(s, 'normal', 'zee', 'El Mu N') u ($1-1):($2-1):3 notitle w image
set output 'grafiken/zmumu/el_mu_n.tikz'
set title '$\Z\to\mu\mu$'
splot sprintf(s, 'normal', 'zmumu', 'El Mu N') u ($1-1):($2-1):3 notitle w image
set output 'grafiken/ztautau/el_mu_n.tikz'
set title '$\Z\to\tau\tau$'
splot sprintf(s, 'normal', 'ztautau', 'El Mu N') u ($1-1):($2-1):3 notitle w image
